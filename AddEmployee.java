package hotelmanagementsystem;
import javax.swing.*;
import java.awt.*;
import java.awt.event.*;

public class AddEmployee extends JFrame implements ActionListener{
    
    JTextField t1,t2,t3,t4,t5,t6;
    JComboBox c1,c2;
    JButton b1,b2;
    
    AddEmployee() {
        
        JLabel name = new JLabel("NAME");
        name.setFont(new Font("Cambria",Font.BOLD,17));
        name.setBounds(60,30,120,30);
        add(name);
        
        t1 = new JTextField();
        t1.setBounds(210, 30, 150, 30);
        add(t1);
        
        JLabel age = new JLabel("AGE");
        age.setFont(new Font("Cambria",Font.BOLD,17));
        age.setBounds(60,80,120,30);
        add(age);
        
        t2 = new JTextField();
        t2.setBounds(210, 80, 150, 30);
        add(t2);
        
        JLabel gender = new JLabel("GENDER");
        gender.setFont(new Font("Cambria",Font.BOLD,17));
        gender.setBounds(60,130,120,30);
        add(gender);
        
        c2 = new JComboBox(new String[] { "Male", "Female" });
	c2.setBounds(210, 130, 150, 30);
        c2.setBackground(Color.white);
        c2.setFont(new Font("Cambria",Font.BOLD,15));
	add(c2);
        
        JLabel job = new JLabel("JOB");
        job.setFont(new Font("Cambria",Font.BOLD,17));
        job.setBounds(60,180,120,30);
        add(job);
        
        String str[] = {"Front Desk Clerks","Porters","House Keeping","Kitchen Staff","Room Service","Waiter/Waitress","Manager","Accountant","Chef"};
        c1 = new JComboBox(str);
        c1.setBounds(210, 180, 150, 30);
        c1.setBackground(Color.white);
        c1.setFont(new Font("Cambria",Font.BOLD,15));
        add(c1);
        
        JLabel salary = new JLabel("SALARY");
        salary.setFont(new Font("Cambria",Font.BOLD,17));
        salary.setBounds(60,230,120,30);
        add(salary);
        
        t3 = new JTextField();
        t3.setBounds(210, 230, 150, 30);
        add(t3);
        
        JLabel phone = new JLabel("PHONE");
        phone.setFont(new Font("Cambria",Font.BOLD,17));
        phone.setBounds(60,280,120,30);
        add(phone);
        
        t4 = new JTextField();
        t4.setBounds(210, 280, 150, 30);
        add(t4);
        
        JLabel aadhar = new JLabel("AADHAR");
        aadhar.setFont(new Font("Cambria",Font.BOLD,17));
        aadhar.setBounds(60,330,120,30);
        add(aadhar);
        
        t5 = new JTextField();
        t5.setBounds(210, 330, 150, 30);
        add(t5);
        
        JLabel email = new JLabel("EMAIL-ID");
        email.setFont(new Font("Cambria",Font.BOLD,17));
        email.setBounds(60,380,120,30);
        add(email);
        
        t6 = new JTextField();
        t6.setBounds(210, 380, 150, 30);
        add(t6);
        
        b1 = new JButton("SUBMIT");
        b1.setBackground(Color.black);
        b1.setForeground(Color.white);
        b1.setBounds(50,450,130,30);
        b1.addActionListener(this);
        b1.setFont(new Font("Cambria", Font.BOLD ,15));
        add(b1);
        
        b2=new JButton("BACK");
        b2.setBackground(Color.BLACK);
        b2.setForeground(Color.WHITE);
        b2.setBounds(225,450,130,30);
        b2.addActionListener(this);
        b2.setFont(new Font("serif", Font.BOLD ,15));
        add(b2);
        
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("hotelmanagementsystem/icons/tenth.jpg"));
        Image i2 = i1.getImage().getScaledInstance(500,700,Image.SCALE_DEFAULT);
        ImageIcon i3 = new ImageIcon(i2);
        JLabel l1 = new JLabel(i3);
        l1.setBounds(400,80,400,450);
        add(l1);
        
        JLabel l2 = new JLabel("ADD EMPLOYEE DETAILS");
        l2.setForeground(Color.darkGray);
        l2.setBounds(420,20,400,40);
        l2.setFont(new Font("Cambria",Font.BOLD,30));
        add(l2);
        
        getContentPane().setBackground(Color.white);
        
        setLayout(null);
        setBounds(350,150,870,550);
        setVisible(true);
    }
    
    public void actionPerformed(ActionEvent ae){
        
        if(ae.getSource()==b1){
            String name = t1.getText();
            String age = t2.getText();
            String salary = t3.getText();
            String phone = t4.getText();
            String aadhar = t5.getText();
            String email = t6.getText();
        
            
            String gender = (String)c2.getSelectedItem();
        
            String job = (String)c1.getSelectedItem();
        
            Conn c = new Conn();
            String str = "insert into Employee values('"+name+"','"+age+"','"+gender+"','"+job+"','"+salary+"','"+phone+"','"+aadhar+"','"+email+"')";
        
            try{
                c.s.executeUpdate(str);
                JOptionPane.showMessageDialog(null,"New Employee Added");
                this.setVisible(false);
                new Dashboard().setVisible(false);
            }catch(Exception e){
                System.out.println(e);
                JOptionPane.showMessageDialog(null, "Invalid information");
            }
        }else if(ae.getSource()==b2){
            new Dashboard().setVisible(true);
            new HotelManagementSystem().setVisible(false);
            this.setVisible(false);
        }
    }
    public static void main(String[] args){
        
        new AddEmployee().setVisible(true);
    }
}